...
---
**2026-09-08** — SKY-11448: the owner-report `dc.html` copies under `companies/<cid>/owner-reports/` ship WITHOUT the dc runtime (`support.js`, babel, react, `assets/`) — served or opened raw they render un-compiled (`{{ }}` bindings, every `sc-if` branch visible). Stage the html in a temp dir next to symlinks of `plans/design-handoff/v2/prototype/*` and open it over `file://` (see `e2e/fidelity/capture-sky11448-scene-crafter-glass.mjs` `stageExternalProto`). Also: a page-level `backdrop-filter` makes that element the containing block for `position:fixed` descendants (modal scrims shrink to the page box) — put the blur on a `::before` with `isolation:isolate` on the page instead.

**2026-09-08** — SKY-11514: an e2e mock that emits `stream:token`/`stream:end` on a fixed timer inside the `stream:start` handler races `useIpcStream`, which only subscribes in a passive effect after the `streamId` commit paints. It passed for months, then went 9/9 red on the GitHub runner the moment the page gained `backdrop-filter` (paint > 30 ms). Never emit on a timer from a mock — record the requester and emit after the test observes the renderer's streaming state (`e2e/helpers/draftStreamMock.ts`). Related: the e2e npm scripts' `--reporter=list,html` replaces the config's html reporter, so `e2e-report/` is never written — upload `test-results/` too or the failure screenshot never leaves the runner.

---
**2026-09-08** — SKY-11336: a path that is legal at BOTH ends of an IPC boundary but means different things is invisible to every guard. The Notes Board renderer passed the ABSOLUTE vault root as `folderPath` where main treats it as vault-RELATIVE: `safeVaultDirIpcJoin` passed it (`path.resolve` collapses an absolute root back to the root, so the traversal check sees nothing wrong), and only `path.join(root, absoluteRoot)` downstream produced the doubled, non-existent folder — silently, into an empty `catch {}`. **Two rules.** (1) Fix a contract mismatch at the end that does NOT already document and enforce it — here the renderer, since main's eight `notesBoard:*` handlers all state "vault-relative" and sandbox on it; normalising in main would have meant two accepted shapes on every channel. (2) A sandbox guard that *sanitises* rather than *rejects* is not a contract check — if `''` and an absolute root are both "valid", nothing downstream can tell a bug from an intent. Related: `listVaultFiles` is RECURSIVE, and was being consumed as an immediate listing on the strength of a comment saying otherwise; `notesBoard.ts`'s own `listImmediateChildren` had the correct `.filter((it) => !it.path.includes('/'))` all along. When a helper's recursion depth matters, assert it in the consumer, not in a comment.

**2026-09-08** — A "create a vault" surface that calls the SKY-11151 primitive (`vault:create-from-options`) with `activate:false` used to get a vault that existed on disk but was invisible to every switcher (Settings cards, rail tiles) and was rejected by `project:switch`, because only the `activate` branch called `addToRecentProjects` — the `recentProjects` list IS the known-vault registry AND the switch allowlist. Registering is now unconditional in that handler; activation stays opt-in. If you add another creation channel, register the pair in recents regardless of activation. (SKY-11452)

---
## SKY-11444 — A boolean prop read only in a `useState` initializer is a one-shot snapshot, not a binding

**2026-09-08** — `NoteViewer`'s legacy `previewMode` prop was consumed exclusively at `useState(resolvedMode)`/`useRef` initializer time (`mode` derives from it once, on mount) — no effect watched later changes, so a parent flipping the prop on an already-mounted instance did nothing visible; it only "took" on the next remount (`key={path}` note switch), which is what made a global toggle look like it silently armed the *next* note instead of the current one. Compounding it: the parent (`DesktopShell`)'s `notePreviewMode` was shell-scoped and never reset on note change, so the one-shot-read illusion held indefinitely. **Fix pattern:** initializer-only reads of a prop are fine for "value at mount," but any prop meant to be *live* needs an explicit effect keyed on that prop (previous-value ref + guarded `setState`) — grep for `useState(<expr using the prop>)` with no companion `useEffect` on the same prop as a smell. Also reset any derived "mode" flag on the identity change that should scope it (here: `openedNotePath`), don't rely on remount alone to clear stale parent state.

---

**2026-09-09** — SKY-11566: when a fidelity gap says a surface has the wrong *value*, check first whether it is reading the wrong *tier*. The Boards zoom pill was filed as "the overlay tier's number is wrong"; the mockup actually never puts that pill on the overlay tier at all (`--glass2` + `blur(20px)` at dc.html:2553, not the frozen `rgba(15,19,33,.97)` dialog recipe), so after OT-1 froze the tier the pill became the only Boards chrome that ignored the Appearance glass slider. The gap list had also attributed the Scene Crafter pill's `rgba(15,19,33,.94)` to Boards — **two views can have visually identical pills with different bindings; grep the view's own `bd*`/`cv*` prefix rather than the nearest matching literal.** Second rule, from BD-5: a child that invents a backdrop is usually standing in for a missing *container*. The mockup's Boards crumb row has no fill because the center pane at dc.html:571 carries `var(--glass)` + `blur(var(--blur)) saturate(150%)` behind it, and our tab shell paints nothing — so copying the mockup's child declaration literally would have put crumb text straight on the wallpaper. Walk up the mockup's ancestors before you copy a child. Third: to flatten a glass recipe for `prefers-reduced-transparency` / `[data-contrast='high']`, declare the fill/blur custom properties on the **parent** and let them inherit — a custom property declared on the element itself out-ranks anything an ancestor-scoped a11y block can say, and a literal `blur(20px)` survives both blocks entirely.

**2026-09-09** — SKY-11540: a Playwright retry runs in a FRESH worker (new `beforeAll`, new Electron app, new vault), so retries of a test that depends on earlier tests in the file can never pass — read only the first attempt's failure. Two lessons from that attempt: (1) a one-shot `textContent()` + `expect(Number(..))` is a non-retrying snapshot; when the value is populated by a post-mount effect (Brainstorm's localStorage draft restore after a tab switch remounts the page) it can read the first frame — use `toHaveText(/^[1-9]\d*$/)`. (2) `useAgentSessions.appendTurns` resolved its target id against `activeSessionId` and silently returned when the store was still awaiting the initial `list` IPC — a reply that finishes within ~200 ms of the Brainstorm mount (mock, or a fast local model) lost the whole exchange from the durable store. Fire-and-forget writes must await the store's init promise, never read a not-yet-loaded id.

**2026-09-09** — SKY-11619: deleting dead code can *sharpen* a guard, and the guard going red is the point. The SKY-11482 orphan check treats a template-literal custom-property name as a **prefix**, so `--ln-graph-node-${category}` covers `--ln-graph-node-scenes`. Dead `TimelineLanes.tsx` contained `var(--n${j.slot}, …)`, which registered the prefix `--n` — one character — and vacuously "defined" every token starting with `--n` repo-wide. `OnboardingWizard.css`'s `--neon-green` / `--neon-pink` had been hiding behind it since the guard shipped, painting frozen literals that never repainted with the theme. **Two rules.** (1) A prefix rule is only as safe as its shortest prefix; when a guard fails right after you delete an unrelated file, look for what the deleted file was accidentally vouching for rather than assuming your deletion broke something. (2) A prefix registered from a *dead* file was never true at runtime anyway — the CSS was never in the bundle — so this class of masking is always a false negative, never a real definition. Grep `(--[\w-]{1,3})\$\{` before trusting that guard's green.

**2026-09-09** — SKY-11636: a "too short" bug in a pattern-matching guard is usually also an "unanchored" bug, and the unanchored half is the expensive one. SKY-11619 traced the orphan guard's blind spot to the one-character prefix `--n`, and the obvious fix is a length floor. But the collector regex `/(--[\w-]+?)\$\{/g` scanned raw source text with no context anchor, and **CSS custom properties and BEM class-name modifiers share the `--` sigil** — so 15 of the 17 prefixes it collected repo-wide came from `className` template literals, not tokens: `` `cvb-card--s${slot + 1}` `` registered `--s`, `` `fmt-btn--bold${…}` `` registered `--bold`, and a string assertion inside `crafterBoardStore.test.ts` registered `--f-g-h-i-j-k-l-`. `--s` alone was vouching for every `--s…` token in the repo and hiding nine genuine orphans. **Three rules.** (1) Anchor a source-scanning regex on the syntactic position that gives the match its meaning — here `var(`, `setProperty(`, or a style-object key, on *both* sides — never on the token's own spelling, because a sigil is rarely unique to one language construct. (2) Enforcing a floor without fixing the anchor would have turned the repo red on ~15 class names an author cannot meaningfully rename; check what a new rule would actually fire on before shipping it. (3) A guard that scans `frontend/src` scans **its own spec**, so its fixtures declare tokens for the whole repo — exclude `.test`/`.spec` from any such sweep. Three of the nine revealed orphans are read with no `var()` fallback, which is worse than the usual frozen literal: the declaration is invalid at computed-value time and the property resolves to `unset` (SKY-11637).

**2026-09-15** — SKY-11742: a mock whose side-effect runs *before its first `await`* silently converts an async race into a passing test. `notesTrash.test.ts` stubbed `shell.trashItem` as `async (t) => { fs.rmSync(t, …) }` — `rmSync` is synchronous and executes before the mock function ever returns a promise, so `flushGroup`'s shallowest-first `fs.existsSync(absPath)` ancestor-skip always observed the parent's removal as already-complete. Real `shell.trashItem` does a genuine async OS move, so in production that check was a no-op and every descendant got its own overlapping trash call. **Rule: in a mock that stands in for async I/O, put the side-effect *after* an `await` boundary — otherwise the mock is synchronous and the test proves nothing about ordering.** Two follow-on rules from fixing it. (1) *Claim before you await.* Serializing a multi-item flush by looking each item up inside the loop leaves every not-yet-reached item live in its registry across each await — reachable by a concurrent Restore IPC or its own armed timer. Detach and disarm ALL of them in one synchronous pass first (`claimGroup`), then process the captured objects. (2) *Serialize by overlap, not wholesale.* Ordering every group put unrelated top-level selections on one critical path, and `emptyTrash`/the app-quit hook both block on it, so one slow OS move delayed window close. Partition by actual subtree overlap (compare each group's minimal roots, not every path) and run the independent chains concurrently. To prove a serialization guard is real, revert only the source file and confirm the new test fails.

**2026-09-15** — SKY-11882: once a directory name becomes **user-chosen**, every renderer-side path-shape guess about it is already wrong — the only question is when someone names a folder to prove it. `MythosVaultsSection`'s `mythosPathFor()` recovered a card's Mythos-bundle root by stripping a hardcoded `Story Vault` suffix. SKY-11451 added a *second* regex when the grouped `Stories/Story Vault` layout shipped, which looked like the fix but was really the tell: the suffix set was being patched to track a value the frontend cannot know. SKY-11169 had made story-vault dirnames arbitrary (`<mythos>/Stories/Second World`, registered in `story-vaults.json`), so the helper matched neither branch, fell through to `return vaultRoot`, and handed the **story-vault subfolder** to `VAULT_SURFACE_TRASH` at `level:'mythos'` — which trashes that path but prunes recents and both registries at the mythos level. Result: `Notes/`, `mythos.json`, `story-vaults.json` and `notes-vaults.json` stranded on disk, with the registries that would have led the user back to them being exactly what got pruned. **Rule: a fallthrough `return input` is not a safe default — it is a *different, plausible* answer, which is how a resolution bug turns into silent data loss instead of a visible error.** Prefer a resolver that returns `null` for "not recognized" and make the caller decide; `mythosRootForStoryVault()` in electron-main already did, and threading its answer through the IPC response (`ProjectListEntry.mythosVaultRoot`) deleted the guess outright rather than adding a third regex. Two corollaries. (1) Put the resolver where the ground truth lives — only main can read the registry, so no amount of frontend cleverness was going to be correct. (2) A *rename* through the picker only changes `displayName`, never `dirName`, so the reachable trigger is **creating** a second story vault with a custom name, not renaming one; when you reproduce a "rename or add" report, confirm which half actually moves the field you care about.

**2026-09-15** — SKY-11887: a filesystem path handed to something that will `import()` it is a **URL**, and on Windows a drive letter is a URL *scheme*. Kokoro TTS was dead in the packaged Windows build because `ort.env.wasm.wasmPaths = wasmDir + path.sep` produced `C:\…\wasm\`; onnxruntime-web resolves `<wasmPaths>ort-wasm-simd-threaded.mjs` and dynamically imports it, and Node's ESM loader rejects everything but the file/data/node schemes — `ERR_UNSUPPORTED_ESM_URL_SCHEME`, protocol `c:`. **Three rules.** (1) The obvious fix is wrong: normalizing separators to `C:/…/wasm/` *still* parses as scheme `c:`. Only `pathToFileURL()` is correct, and it is correct on every platform, so don't platform-branch it. (2) The POSIX case passed by **accident** — `import('/abs/path.mjs')` resolves as a relative URL against the parent module and happens to land on the right file — which is exactly why Linux and macOS CI can never see this class of bug. Prove the mechanism instead: `new URL()` parses a drive-letter path as scheme `c:` on Linux too, so a hardcoded-string assertion is a real, platform-independent regression guard. (3) The branch broke because **no test ever set it** — `kokoro.test.ts`'s asset fixture omitted `wasmDir`, so the integration tests used onnxruntime-web's node_modules-relative resolution and the packaged-only code path had zero coverage. When a config field exists solely to serve the packaged build, the asset-gated integration fixture must populate it.
