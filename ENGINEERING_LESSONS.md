...
**2026-09-08** — SKY-11504: a design token derived in CSS from another token is only equal to the engine's arithmetic while that other token is UNSATURATED. The Liquid Neon hairline border is `hexA(c1, (.3 + .4*I) / 2)`; `--b1` is `hexA(c1, .3 + .4*I)`, which clamps to `1.000` at the shipped default intensity, so PR #1465's `color-mix(--b1 50%, transparent)` shipped alpha `.500` against the mockup's `.550` — correct at low intensity, silently wrong at the default. **Rule:** halve/scale a token in the engine, before the clamp, not in CSS after it; `calc()`/`color-mix()` can only ever see the clamped output. Same class: `calc(--gr / 2)` ≠ the mockup's `Math.round(glowR/2)` on odd radii. Corollary for evidence — "the border repaints when the accent changes" does NOT prove the value is right; assert the computed border equals the engine's own token (resolve it through a throwaway probe element so both sides come back in the same colour syntax).
