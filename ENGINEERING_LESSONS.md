...
**2026-09-09** — SKY-11192: an off-by-default flag that only swaps the *view* is not off. Brainstorm's legacy board kept its own `loadBrainstormBoard` / debounced `saveBrainstormBoard` effects running under the unified-board flag; the flag chose which canvas rendered, so those effects looked harmless. They were not: the load effect writes the file back on read (`if (loaded !== null) void saveBrainstormBoard(next)`), which **resurrected the exact file the one-time migration had just parked as `.migrated`** — so the next launch found a board file again, migrated it again, and parked `.migrated (2)`, `(3)`, … forever. It passed every unit test (the migration is genuinely idempotent in isolation) and was only caught by the E2E asserting the source file is *gone* after migration, not merely that the `.migrated` copy exists. **Three rules.** (1) When a flag retires a data model, make the old store **inert**, not merely unrendered — gate the effects that read and write it, not just the JSX. (2) Assert the negative in a migration test: `.migrated` existing proves the rename ran, only `!existsSync(source)` proves nothing put it back. (3) The same sweep catches dead controls — `+ Idea` and the board search box still rendered under the flag and mutated the retired in-memory model, so they were silently no-ops; when you swap a surface behind a flag, grep every control that writes to the old model, not just the component you replaced.
