...
import { applyTheme, applyLiquidNeonTokens, resetLiquidNeonTokens, LIQUID_NEON_DEFAULTS, DEFAULT_BG_GRADIENT, contrastRatio, enforceContrastFloor, applyPageBackgroundTokens, PAGE_BACKGROUND_DEFAULTS, pageBackgroundContrastRatio, type ThemeMode } from './theme';
...
export default function SettingsPanel({ onClose, onSaved, focusPrefs, onFocusPrefsChange }: Props) {
  ...
  // SKY-7422: registration tokens for voice local-engine path changes (MYT-788).
  const [sttBinaryToken, setSttBinaryToken] = useState<string | null>(null);
  const [sttModelToken, setSttModelToken] = useState<string | null>(null);

  // SKY-7422: browse handlers for STT local binary / model.
  const handlePickSttBinary = useCallback(async () => {
    const result = await window.api.voicePickBinary('stt-binary');
    if (result.cancelled || !result.path) return;
    setSttBinaryToken(result.registrationToken);
    setSettings((p) => ({ ...p, stt: { enabled: true, provider: 'local', ...p.stt, localBinaryPath: result.path! } }));
    setSavedOk(false);
  }, []);

  const handlePickSttModel = useCallback(async () => {
    const result = await window.api.voicePickBinary('stt-model');
    if (result.cancelled || !result.path) return;
    setSttModelToken(result.registrationToken);
    setSettings((p) => ({ ...p, stt: { enabled: true, provider: 'local', ...p.stt, localModelPath: result.path! } }));
    setSavedOk(false);
  }, []);
  ...
  }, [settings, apiKeyInput, apiKeyDirty, apiKeyError, providerKind, providerModel, providerApiKey, providerApiKeyDirty, providerBaseUrl, telemetryEnabled, lg, bgPreviewUrl, pageBg, onSaved, buildAgentProviderConfig, sttBinaryToken, sttModelToken]);
  ...
  // SKY-2308: Vault health check state + handlers
            ...
            <div className="settings-field settings-field-inline">
              <label className="settings-toggle" htmlFor="archive-continuity-enabled">
                <input
                  ...
                  onChange={(e) => setSettings((p) => ({ ...p, archiveContinuityEnabled: e.target.checked }))}
                />
                <span className="settings-toggle-track" />
            ...
            >
              <div className="settings-field settings-field-inline">
                <label className="settings-toggle" htmlFor="archive-scan-on-save">
                  <input
                    ...
                    onChange={(e) => setSettings((p) => ({ ...p, archiveScanOnSave: e.target.checked }))}
                  />
                  <span className="settings-toggle-track" />
              ...
              <div className="settings-field">
                ...
                <select
                  ...
                  onChange={(e) => setSettings((p) => ({ ...p, archiveScanScope: e.target.value as AppSettings['archiveScanScope'] }))}
                >
              ...
              <div className="settings-field">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setSettings((p) => ({ ...p, archiveScanInterval: val }));
                  }}
                >
              ...
              <div className="settings-field">
                ...
                <select
                  ...
                  onChange={(e) => setSettings((p) => ({ ...p, archiveMinSeverity: e.target.value as AppSettings['archiveMinSeverity'] }))}
                >
              ...
              <div className="settings-field settings-field-inline">
                <label className="settings-toggle" htmlFor="archive-check-character-drift">
                  <input
                    ...
                    onChange={(e) => setSettings((p) => ({ ...p, archiveCheckCharacterDrift: e.target.checked }))}
                  />
                  <span className="settings-toggle-track" />
              ...
              <div className="settings-field settings-field-inline">
                <label className="settings-toggle" htmlFor="archive-check-location-mismatch">
                  <input
                    ...
                    onChange={(e) => setSettings((p) => ({ ...p, archiveCheckLocationMismatch: e.target.checked }))}
                  />
                  <span className="settings-toggle-track" />
              ...
              <div className="settings-field settings-field-inline">
                <label className="settings-toggle" htmlFor="archive-check-factual-contradict">
                  <input
                    ...
                    onChange={(e) => setSettings((p) => ({ ...p, archiveCheckFactualContradict: e.target.checked }))}
                  />
                  <span className="settings-toggle-track" />
