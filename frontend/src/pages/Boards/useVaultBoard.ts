...
export interface VaultBoard {
   ...
   * from a board shows up in the tree and vice versa. Read here because it
   * arrives on the same load as the listing; the SETTER lives with whichever
   * surface offers the icon picker (today only the Notes Board tab).
   */
  iconMap: Record<string, VaultIconEntry>;
  setIconMap: React.Dispatch<React.SetStateAction<Record<string, VaultIconEntry>>>;
