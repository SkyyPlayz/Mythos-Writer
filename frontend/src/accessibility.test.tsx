...
describe('Accessibility — LeftRail nav + panel zone (WCAG 4.1.2)', () => {
  ...
  it('default layout (entities panel) — no axe violations', async () => {
    const { container } = render(withDrag(<LeftRail {...DEFAULT_LEFT_RAIL_PROPS} />));
    await act(async () => {});
    const results = await axe(container);
    expect(results).toHaveNoViolations();
  });
  ...
  it('with stories panel — no axe violations', async () => {
    ...
    const { container } = render(
      withDrag(<LeftRail {...DEFAULT_LEFT_RAIL_PROPS} leftSidebarLayout={layout} />),
    );
    await act(async () => {});
    const results = await axe(container);
    expect(results).toHaveNoViolations();
  ...
  it('nav zone renders 4 nav icon buttons with aria-label and aria-pressed', async () => {
    const { container } = render(withDrag(<LeftRail {...DEFAULT_LEFT_RAIL_PROPS} activeView="brainstorm" />));
    await act(async () => {});
