...
export default function DesktopShell({ initialSettings }: { initialSettings?: AppSettings } = {}) {
  ...
  // once the paragraph exists in the DOM.
  const [manuscriptCaretRequest, setManuscriptCaretRequest] =
    useState<{ blockId: string; seq: number } | null>(null);
  ...
  // M6: panel system removed from GRS. Kept for potential undo/redo restore paths.
  const handleGrsPanelsChange = useCallback((panels: PanelConfig[]) => {
    setGrsPanels(panels);
    persistGrsSettings({ panels });
  }, [persistGrsSettings]);
  // Suppress unused-locals warning: retained for restore paths, not wired to UI post-M6.
  void handleGrsPanelsChange;
  ...
  // M6: panel system removed; retained for handleFloatPanel which still calls handleTabBarDrop.
  ...
  //
  // Generates the chapter/scene client-side and goes through updateManifest,
  // the same as createChapter/createScene above — NOT window.api.chapterCreate
  // /sceneCreate, which re-read the manifest from disk. Those IPC calls raced
  // a freshly-created story that only exists in React state and hadn't hit
  // disk yet (scheduleManifestSave debounces 900ms), throwing "Story not
  // found" the moment a scene was created right after the story itself.
  const createSceneFromSetup = useCallback(async (setup: CrafterSetup) => {
    ...
    if (workingStory.chapters.length === 0) {
      ...
      chapter = {
        ...
        path: `stories/${workingStory.id}/chapters/${chapterId}`,
        order: 0, scenes: [], createdAt: now(), updatedAt: now(),
      };
      workingStory = appendChapterToStory(workingStory, chapter);
    } else {
      const preferred = selectedChapter && workingStory.chapters.find((c) => c.id === selectedChapter.id);
  ...
  // Story Writer at Full Book depth with the caret in that paragraph
  ...
  const createStory = useCallback(async () => {
    ...
    const scene: Scene = {
      id: sceneId, title: 'Untitled Scene',
      path: `stories/${storyId}/chapters/${chapterId}/scenes/${sceneId}.md`,
      order: 0, chapterId, storyId,
      blocks: [{ id: blockId, type: 'prose', content: '', order: 0, updatedAt: stamp }],
      draftState: 'in-progress', createdAt: stamp, updatedAt: stamp,
    };
    const chapter: Chapter = {
      id: chapterId, title: 'Chapter 1',
      path: `stories/${storyId}/chapters/${chapterId}`,
      order: 0, scenes: [scene], createdAt: stamp, updatedAt: stamp,
    };
    const part: Part = {
      id: generateId(), title: '', order: 0, note: [],
  ...
  useEffect(() => {
    ...
    const { sceneId, cursorLine, openAtDepth } = appSettings.lastOpenedScene;
    for (const story of stories) {
      for (const chapter of story.chapters) {
        const scene = chapter.scenes.find((sc) => sc.id === sceneId);
        if (scene) {
          ...
          // M3 (SKY-9021): onboarding's create-story lands at Full Book depth
          // with the caret in the scaffolded scene's first paragraph — same
          // contract as every other create entry point. One-shot: the next
          // sessionSaveScene writes lastOpenedScene without openAtDepth.
          if (openAtDepth === 'book') {
            setViewDepth('book');
            const firstBlock = scene.blocks[0];
            if (firstBlock) {
              caretSeqRef.current += 1;
              setManuscriptCaretRequest({ blockId: firstBlock.id, seq: caretSeqRef.current });
            }
          }
          return;
        }
  ...
  const handleManuscriptRenameStory = useCallback((title: string) => {
    if (!selectedStory || title === selectedStory.title) return;
    const renamed: Story = { ...selectedStory, title, updatedAt: now() };
    updateManifest(stories.map((st) => (st.id === renamed.id ? renamed : st)));
    refreshManuscriptSelection(renamed);
  }, [selectedStory, stories, updateManifest, refreshManuscriptSelection]);
  ...
  // the left sidebar (GH #633 home). SKY-11244: Read is no longer wired here
  // — ManuscriptView owns the Story Editor's one real Read control (its own
  // toolbar button + reader dock, GAP P0#4), and NoteViewer now supplies its
  // own working Read handler directly (see richEditorToolbarActions there).
  ...
  // is supplied per-instance by NoteViewer itself (SKY-11244).
