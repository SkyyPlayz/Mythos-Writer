    ...
    for (const layout of layouts) {
      ...
      for (const ch of layout.chapters) {
        for (const s of ch.scenes) {
          type DraftSource = {
            savedAt: string;
            /** Full version ts (includes monotonic seq) — tiebreaker when savedAt ties. */
            _sortKey: string;
            content: string;
          ...
          for (const v of listVersions(opts.sourceStoryVault, s.scene.id, {
            ...
            // v.ts includes the monotonic _seq suffix — use it as a tiebreaker so that
            // two versions saved within the same millisecond stay in creation order.
            sources.push({ savedAt, _sortKey: v.ts, content: v.content, intent: v.intent });
          }
          try {
            for (const snap of listSnapshots(opts.sourceStoryVault, s.scene.id)) {
              sources.push({
                savedAt: snap.createdAt,
                _sortKey: snap.createdAt,
          ...
          for (const row of dbRows.sceneSnapshots) {
            ...
            const rowSavedAt = new Date(row.created_at).toISOString();
            sources.push({
              savedAt: rowSavedAt,
              _sortKey: rowSavedAt,
              content: row.content,
