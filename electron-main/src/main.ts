...
const handlers: IpcHandlers = {
  ...
  [IPC_CHANNELS.NOTES_VAULT_LIST]: (payload: VaultListPayload): VaultListResponse => {
    ...
    // location and item/system signals for Scene Crafter's category pickers
    // and vault-wide fallbacks. Compute all here — one bounded read per note
    // during the listing, after filtering so story internals are never
    // opened — instead of per-card IPC round-trips from the renderer (an N+1
    // over the vault).
    return {
      items: filtered.map((item) => {
        if (item.isDirectory || !/\.md$/i.test(item.path)) return item;
        const { excerpt, characterTag, locationTag, itemTag } = readNoteListingMeta(
          path.join(listedRoot, item.path),
        );
