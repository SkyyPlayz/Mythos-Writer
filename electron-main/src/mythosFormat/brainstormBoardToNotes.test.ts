...
describe('SKY-11192 — migrating the board into notes', () => {
  ...
  // SKY-11451 moved NEW vaults to a grouped `<root>/Notes/Notes Vault` layout.
  // A vault scaffolded before that is still flat, and its bound notes root is
  // the one the user actually opens — so the migration has to file into the
  // root it is HANDED, not one recomputed from the mythos root. Without the
  // override, a legacy vault's ideas land in a grouped folder nobody looks at.
  it('files into the bound notes root, not one recomputed from the mythos root', () => {
    const flatNotesRoot = path.join(mythosRoot, 'Notes Vault');
    fs.mkdirSync(flatNotesRoot, { recursive: true });
    seedBoard([{ title: 'Midpoint Reversal', cat: 'beats', desc: 'The goal changes.' }]);

    const res = migrateBrainstormBoardToNotes(mythosRoot, flatNotesRoot);

    expect(res.migrated).toBe(true);
    expect(fs.existsSync(path.join(flatNotesRoot, 'Plot & Story/Midpoint Reversal.md'))).toBe(true);
    // ...and nothing was filed into the grouped path the helper would derive.
    expect(fs.existsSync(path.join(notesRoot, 'Plot & Story/Midpoint Reversal.md'))).toBe(false);
  });
