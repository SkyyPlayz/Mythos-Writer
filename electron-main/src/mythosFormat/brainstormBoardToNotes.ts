 ...
 *
 * @param notesRootOverride the notes vault the app is actually bound to.
 *   SKY-11451 moved new vaults to a grouped `<root>/Notes/Notes Vault` layout,
 *   so `notesVaultRootFor` is only correct for vaults scaffolded AFTER it —
 *   recomputing the path would file a legacy flat vault's ideas into a brand
 *   new empty folder the user never opens. The caller already knows the bound
 *   root (`getNotesVaultRoot()`), which is right for both layouts; the derived
 *   path stays as the fallback for callers that have nothing better.
 */
export function migrateBrainstormBoardToNotes(
  mythosRoot: string,
  notesRootOverride?: string,
): BoardToNotesResult {
  const sourcePath = path.join(agentVaultRootFor(mythosRoot), BRAINSTORM_BOARD_RELPATH);
  ...
  const notesRoot = notesRootOverride ?? notesVaultRootFor(mythosRoot);
