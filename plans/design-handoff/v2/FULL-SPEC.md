...
11. **Visual-fidelity gate (SKY-8962):** the surface has been compared side-by-side, live, against the CURRENT export of `plans/design-handoff/v2/prototype/Mythos Writer - Liquid Neon.dc.html` (not a memory of it, not a screenshot from a prior pass) and the delta recorded — matches, or named gaps with a follow-up issue. Run [`scripts/check-prototype-freshness.sh`](../../../scripts/check-prototype-freshness.sh) first so the "current export" you're comparing against isn't itself stale (see PR #1156 / SKY-8962 — M25 was built against a copy that predated an owner refresh). No sign-off from an old screenshot or from what the surface "usually looks like."
