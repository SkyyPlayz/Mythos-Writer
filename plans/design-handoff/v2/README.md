...
> **START HERE → `FULL-SPEC.md`** — the complete ground-up build specification (assume nothing is built). The prototype in `prototype/` is the pixel reference; the spec's acceptance checklist is the definition of done. Older docs below (DESIGN-SPEC, GAP-REPORT-v2) are historical context only.
