...
- Optional **Mythos-hosted cloud storage subscription** (Monetization Phase 3, post-MVP): in-app sync between devices without relying on OS-level desktop syncers. Tied to the paid subscription tier and the future mobile companion. SKY-11804 removed the in-app branded cloud/sync surface, so until this phase ships the app offers no sync feature of any kind; a vault is a plain folder the user is free to place wherever they like.
