...
| Path | What it is |
| --- | --- |
| `design-assets/wallpapers/originals/<setKey>/<setKey>-src-NN.png` | Where the owner's PNGs are restaged to rebuild. Git-ignored — see below. |
| `design-assets/wallpapers/PROVENANCE.json` | Per original: filename, pixel dimensions, bytes, SHA-256, owner folder/filename, and the WebP it produced. |
...
### The originals are not in git

68.2 MB of PNGs would sit in history forever, so Ivy ruled on 2026-09-09
(SKY-11675) that they stay off the repo. `design-assets/wallpapers/originals/`
is in `.gitignore`; the owner holds the files, and SKY-11600 tracks regenerating
them at higher resolution.

`PROVENANCE.json` is what stands in for them. Because it carries a SHA-256 and
the pixel dimensions of every original, a restaged copy can be proven to be the
same file a given wallpaper was cropped from — which is the only thing the
blobs were being kept for.

### Rebuilding

Restage the owner's PNGs as `originals/<setKey>/<setKey>-src-NN.png` and check
each one's `sha256sum` against its `PROVENANCE.json` entry. A mismatch means the
file is not the source the committed crop was taken from, and the rebuild will
not reproduce the shipped bytes. Then edit the manifest and run:
