// SKY-10529 — M9 FINAL fresh-profile re-run (follow-up to SKY-10508, gated on
// SKY-10499 / PR #1256). Same fixtures and checks as sky10508-m9-repass.mjs,
// with ONE deliberate difference: the seeded app-settings.json carries NO
// rightSidebarVisible / rightSidebarWidth / rightSidebarPanels keys — the
// layout is genuinely fresh, so the right-panel tab strip must appear via
// migrateV1Layout's `visible ?? true` default (PR #1256), not a force.
...
//
// Usage: node e2e/fidelity/sky10529-fresh-repass.mjs [--proto] [--app] [--montage]
...
const OUT = outDir('sky10529-fresh-repass');
...
function seedFixture() {
  const userData = fs.mkdtempSync(path.join(os.tmpdir(), 'mythos-10529-ud-'));
  const vaultDir = fs.mkdtempSync(path.join(os.tmpdir(), 'MythosVault-10529-'));
  const notesVaultDir = fs.mkdtempSync(path.join(os.tmpdir(), 'MythosNotes-10529-'));
...
if (doApp) {
  ...
  // SKY-10499 proof — with NO rightSidebar* keys in the seeded settings, the
  // tab strip must already be mounted before any click, and Getting Started
  // must not occupy the panel as a full-height aside.
  const bootRight = await page.evaluate(() => {
    const out = [];
    document.querySelectorAll('body *').forEach((el) => {
      const r = el.getBoundingClientRect();
      if (r.left < 1500 || r.width < 4 || el.children.length) return;
      const t = (el.innerText || '').trim();
      if (t) out.push(t);
    });
    return { text: [...new Set(out)].join('\n'), gsAside: !!document.querySelector('.gs-aside') };
  });
  fs.writeFileSync(`${OUT}/app-fresh-boot.txt`, bootRight.text);
  check('fresh: tab strip mounted with NO rightSidebar force',
    ['Assistant', 'Scenes', 'Notes', 'References'].every((t) => bootRight.text.includes(t)));
  check('fresh: no full-height Getting Started aside (.gs-aside gone)', !bootRight.gsAside);
  await page.screenshot({ path: `${OUT}/app-fresh-boot.png`, clip: { x: 1520, y: 0, width: 400, height: 1080 } });
